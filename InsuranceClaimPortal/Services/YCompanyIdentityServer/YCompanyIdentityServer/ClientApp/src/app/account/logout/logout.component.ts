...
export class LogoutComponent {
  ...
  onClick() {
    this.response$ = this.http.post<{ prompt: boolean, iFrameUrl: string, postLogoutRedirectUri: string }>(`/api/logout?logoutId=${this.logoutId}`, null).pipe(map(value => ({ ...value, prompt: false })));
  }
}
